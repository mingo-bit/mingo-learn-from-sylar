#ifndef MINGO_HTTP_SERVLET_H
#define MINGO_HTTP_SERVLET_H
...
public:
     ...
     * @return 是否处理成功
     */
    virtual int32_t handle(mingo::http::HttpRequest::ptr request
                   , mingo::http::HttpResponse::ptr response
                   , mingo::http::HttpSession::ptr session) = 0;
...
public:
    ...
    /// 函数回调类型定义
    typedef std::function<int32_t (mingo::http::HttpRequest::ptr request
                   , mingo::http::HttpResponse::ptr response
                   , mingo::http::HttpSession::ptr session)> callback;
    ...
    virtual int32_t handle(mingo::http::HttpRequest::ptr request
                   , mingo::http::HttpResponse::ptr response
                   , mingo::http::HttpSession::ptr session) override;
private:
    /// 回调函数
    callback m_cb;
};
