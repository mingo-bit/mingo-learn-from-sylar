...
static mingo::Logger::ptr g_http_conn_logger = GET_LOGGER("system");
...
HttpConnection::~HttpConnection() {
    LOG_DEBUG(g_http_conn_logger, "HttpConnection::~HttpConnection");
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            LOG_DEBUG(g_http_conn_logger, "content_len=" + std::to_string(client_parser.content_len));
    ...
    if(!body.empty()) {
        ...
        LOG_DEBUG(g_http_conn_logger, "content_encoding: " + content_encoding
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri
    ...
    if(!turi) {
        LOG_ERROR(g_http_conn_logger, "invalid uri=" + uri);
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = mingo::GetCurrentMS();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            LOG_ERROR(g_http_conn_logger, "get addr fail: " + m_host);
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            LOG_ERROR(g_http_conn_logger, "create sock fail: " + addr->toString());
            return nullptr;
        }
        if(!sock->connect(addr)) {
            LOG_ERROR(g_http_conn_logger, "sock connect fail: " + addr->toString());
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= mingo::GetCurrentMS())
